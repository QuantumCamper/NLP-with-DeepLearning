...
def create_rnn(seq, hidden_size=HIDDEN_SIZE):
    ...
    init_state = tf.placeholder_with_default(cell.zero_state(tf.shape(seq)[0], tf.float32), [None, hidden_size])
    # this line to calculate the real length of seq
    # all seq are padded to be of the same length which is NUM_STEPS
    length = tf.reduce_sum(tf.reduce_max(tf.sign(seq), 2), 1)
    output, out_state = tf.nn.dynamic_rnn(cell, seq, length, init_state)
    return output, init_state, out_state
    ...
    with tf.Session() as sess:
        ...
        for batch in read_batch(read_data(DATA_PATH, vocab)):
            ...
            if (iteration + 1) % SKIP_STEP == 0:
                ...
                start = time.time()
                saver.save(sess, 'checkpoints/arvix', iteration)
            iteration += 1
...
def main():
    ...
    seq = tf.placeholder(tf.int32, [None, None], name='seq')
