...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        embedding_pre = tf.constant(self.pretrained_embeddings,name='embedding_pre')
        print embedding_pre.shape
        embed_lookup = tf.nn.embedding_lookup(embedding_pre, self.input_placeholder)
        print embed_lookup.shape
        embeddings = tf.reshape(embed_lookup,[-1, self.config.n_features*self.config.embed_size])  
    ...
    def add_prediction_op(self):
        ...
        xavier_w = xavier_weight_init([self.config.n_features*self.config.embed_size, self.config.hidden_size])
        xavier_u = xavier_weight_init([self.config.hidden_size, self.config.n_classes])
        W = tf.Variable(xavier_w, name='W')
        U = tf.Variable(xavier_u, name='U')
