...
class SequencePredictor(Model):
    ...
    def add_prediction_op(self): 
        ...
        outputs = tf.unstack(outputs, axis=1)[-1]
        print(outputs)
    ...
    def add_training_op(self, loss):
        ...
        grads, global_norms = tf.clip_by_global_norm(tf.gradients(loss, params),
                                                     clip_norm=self.config.max_grad_norm)
        self.grad_norm = global_norms
...
if __name__ == "__main__":
    ...
    ARGS = parser.parse_args(['dynamics'])
