...
class GRUCell(tf.contrib.rnn.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            W_r = tf.get_variable('W_r', dtype=tf.float32, shape=[self.state_size, self.state_size],
                                  initializer=tf.contrib.layers.xavier_initializer)
            U_r = tf.get_variable('U_r', dtype=tf.float32, shape=[self.input_size, self.state_size],
                                  initializer=tf.contrib.layers.xavier_initializer)
            b_r = tf.get_variable('b_r', dtype=tf.float32, shape=[self.state_size])

            W_z = tf.get_variable('W_z', dtype=tf.float32, shape=[self.state_size, self.state_size],
                              initializer=tf.contrib.layers.xavier_initializer)
            U_z = tf.get_variable('U_z', dtype=tf.float32, shape=[self.input_size, self.state_size],
                              initializer=tf.contrib.layers.xavier_initializer)
            b_z = tf.get_variable('b_z', dtype=tf.float32, shape=[self.state_size])

            W_o = tf.get_variable('W_o', dtype=tf.float32, shape=[self.state_size, self.state_size],
                              initializer=tf.contrib.layers.xavier_initializer)
            U_o = tf.get_variable('U_o', dtype=tf.float32, shape=[self.input_size, self.state_size],
                              initializer=tf.contrib.layers.xavier_initializer)
            b_o = tf.get_variable('b_o', dtype=tf.float32, shape=[self.state_size])
